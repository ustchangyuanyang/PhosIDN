...
def model_phosidn(X_train, ppi_features_train, y_train,
              nb_epoch=80,weights=None):

    nb_classes = 2
    img_dim1 = X_train.shape[1:]
    img_dim2 = ppi_features_train.shape[1:]
    ...
    # Construct model #
    ###################
    from phosnet import PhosIDN
    model = PhosIDN(nb_classes, nb_layers,img_dim1, img_dim2,init_form, nb_dense_block,
    ...
    if nb_epoch > 0 :
      model.fit([X_train, ppi_features_train,np.ones(X_train.shape[0])], y_train, batch_size=nb_batch_size,
